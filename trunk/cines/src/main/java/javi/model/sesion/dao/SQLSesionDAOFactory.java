package javi.model.sesion.dao;

import es.udc.fbellas.j2ee.util.configuration.ConfigurationParametersManager;
import es.udc.fbellas.j2ee.util.exceptions.InternalErrorException;

public final class SQLSesionDAOFactory {

    private final static String DAO_CLASS_NAME_PARAMETER =
        "SQLSesionDAOFactory/daoClassName";

    private final static Class daoClass = getDAOClass();
    
    private SQLSesionDAOFactory() {}
    
    private static Class getDAOClass() {
    
        Class theClass = null;
    
        try {
        
            String daoClassName = 
                ConfigurationParametersManager.getParameter(
                    DAO_CLASS_NAME_PARAMETER);
            
            theClass = Class.forName(daoClassName);
            
        } catch (Exception e) {
            e.printStackTrace();
        }
        
        return theClass;
        
    }
    
    public static SQLSesionDAO getDAO() throws InternalErrorException {
        
        try {        
            return (SQLSesionDAO) daoClass.newInstance();
        } catch (Exception e) {
            throw new InternalErrorException(e);
        }
        
    }
    
    
}
