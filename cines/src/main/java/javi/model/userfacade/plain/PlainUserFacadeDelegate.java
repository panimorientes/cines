...
import javi.model.userfacade.plain.actions.LiberarTicketsAction;
...
public class PlainUserFacadeDelegate implements UserFacadeDelegate {
	...
	//TODO el try-catch este no se como ponerlo
	public void liberarTickets() throws ModelException, InternalErrorException {

		LiberarTicketsAction action = new LiberarTicketsAction(this.loginName,
				this.carroCompra);
	...
	public void registerUser(String loginName, String clearPassword, Long cp,
			String ciudad, String direccion, Long numero,
		...
		try {
			...
			RegisterUserAction action = new RegisterUserAction(loginName,
					clearPassword, cp, ciudad, direccion, numero,
	...
	public LoginResultVO login(String loginName, String password,
		...
		try {
			...
			LoginResultVO loginResultVO = (LoginResultVO) PlainActionProcessor
	...
	public UserProfileVO findUserProfile() throws InternalErrorException {
		...
		try {
			...
			return (UserProfileVO) PlainActionProcessor.process(
	...
	public DireccionVO findUserDireccion() throws InternalErrorException {
		...
		try {
			...
			FindUserDireccionAction action = new FindUserDireccionAction(
					loginName);

			return (DireccionVO) PlainActionProcessor.process(getDataSource(),
					action);
	...
	public void updateUserProfileDetails(
			UserProfileDetailsVO userProfileDetailsVO, Long cp, String ciudad,
			String direccion, Long numero) throws InternalErrorException {

		try {

			UpdateUserProfileDetailsAction action = new UpdateUserProfileDetailsAction(
					loginName, userProfileDetailsVO, cp, ciudad, direccion,
					numero);
	...
	public void registrarTarjeta(Long tarjeta) throws InternalErrorException {

		try {

			RegistrarTarjetaAction action = new RegistrarTarjetaAction(
					loginName, tarjeta);
	...
	public void anadirPedido(Long idPedido, Calendar fecha)
		...
		try {
			...
			AnadirPedidoAction action = new AnadirPedidoAction(idPedido, fecha,
					loginName);
	...
	public void comprarM(long id) throws InternalErrorException {
		try {
			...
			carroCompra.addProducto((MerchandisingVO) PlainActionProcessor
					.process(getDataSource(), action));
		} catch (Exception e) {
			throw new InternalErrorException(e);
		}
	...
	public void comprarD(String id) throws InternalErrorException {
		try {
			...
			carroCompra.addProducto((dvdVO) PlainActionProcessor.process(
					getDataSource(), action));
		} catch (Exception e) {
			throw new InternalErrorException(e);
		}
	...
	public TarjetaVO findUserTarjeta() throws InternalErrorException {
		...
		try {
			...
			return (TarjetaVO) PlainActionProcessor.process(getDataSource(),
					action);
		} catch (ModelException e) {
			throw new InternalErrorException(e);
		} catch (InternalErrorException e) {
